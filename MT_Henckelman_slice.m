function [parameters,resnorm] = MT_Henckelman_slice(folder,omega1,scan1,num_power,scan2)
...
options = optimoptions('lsqnonlin','FunctionTolerance',1e-12, ...
                       'MaxIterations',2000,'OptimalityTolerance',1e-12, ...
                       'StepTolerance',1e-12);
[parameters, resnorm, residuals, ~, ~, ~, jacobian] = ... 
lsqnonlin(@superLorentziancost,[20,0.5,8e-6,2,60],[0 0 0 0 0],[],options,...
           omega1,offsets,data);
savedir=[folder,'/Results'];
if  ~exist(savedir)
    mkdir(savedir)
